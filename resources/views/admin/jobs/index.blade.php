                                ...
                                @if (!auth()->user()->isStudent())
                                    <td>
                                        @php
                                            ...
                                            $student = DB::select('select * from users where id =' . $studentId);
                                            // dd($candidateName[0]);
                                        @endphp
                                        @foreach ($student as $student)
                                            <a
                                                href="{{ route('teacherprofile', $student->id) }}">{{ $student->name ?? '' }}</a>
                                        @endforeach
                                    ...
                                    @if ($job->attachments)
                                        @foreach ($job->attachments as $key => $media)
                                            <a class="badge badge-success"
                                                href="{{ route('admin.jobs.downloadMedia', $media) }}" target="_blank">
                                                view file
                                            </a>
                                        @endforeach
                                    @endif
                                    ...
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        @if (auth()->user()->isTeacher())
                                            <a class="btn btn-xs btn-info"
                                                href="{{ route('admin.proposals.create') }}?job_id={{ $job->id }}">
                                                create proposal
                                            </a>
                                        @endif
                                    </div>
